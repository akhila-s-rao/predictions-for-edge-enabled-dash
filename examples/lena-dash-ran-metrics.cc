...
#include <sstream>
#include <vector>
#include "lena-dash-ran-metrics.h"
...
main (int argc, char *argv[])
{
  ...
  lteHelper->SetSchedulerType ("ns3::PfFfMacScheduler"); // proportional fair. Provides throughput fairness. 
  ...
  NS_LOG_LOGIC ("randomly allocating macro UEs in " << macroUeBox << " speedMin "
                                                    ...
                                                    << outdoorUeMaxSpeed);
  std::cout << Simulator::Now () << "randomly allocating macro UEs in " << macroUeBox << " speedMin "
                                                    << outdoorUeMinSpeed << " speedMax "
                                                    << outdoorUeMaxSpeed
                                                    << std::endl;
  ...
  if (epc)
    {
      ...
      if (useUdp)
      ...
      else
        {
          ...
          //startTimeSeconds->SetAttribute ("Min", DoubleValue (0.1));
          //startTimeSeconds->SetAttribute ("Max", DoubleValue (MAX_START_TIME_DELAY));
        }
      ...
      for (uint32_t u = 0; u < ues.GetN (); ++u)
        {
          ...
          for (uint32_t b = 0; b < numBearersPerUe; ++b)
            {
              ...
              else if (useDash) // use DASH and TCP
                {
		  ...
		  "VideoId", UintegerValue ((u % numVideos) + 1));
              ...
              //Time startTime = Seconds (2*u+1);
              // moved this to outside the ue loop. It should not be in the loop.
  ...
  *parmStream->GetStream() << "Locations of homeEnbs" << std::endl;
  ...
  // Mobility model is installed on macroUes
  //AsciiTraceHelper ascii;
  //mobility.EnableAsciiAll (ascii.CreateFileStream ("mobility-trace-example.mob"));

  /*Ptr<Node> ue_node = macroUes.Get(2);
  Ptr<MobilityModel> mob_model = ue_node->GetObject<MobilityModel>();
  mob_model->TraceConnectWithoutContext("CourseChange", MakeCallback(&LocationTrace));
  */

  // connect mobility trace source 
  //Config::Connect ("/NodeList/*/DeviceList/*/$ns3::MobilityModel/CourseChange",
  
  //std::ostringstream oss;
  //uint32_t nodeid = 10;
  //oss << "/NodeList/*/$ns3::MobilityModel/CourseChange";
  //Config::Connect (oss.str (), MakeCallback (&LocationTrace));

  //Config::Connect ("/NodeList/*/$ns3::MobilityModel/CourseChange", MakeCallback (&LocationTrace));
  ...
  //Config::Connect ("/NodeList/12/$ns3::MobilityModel/CourseChange",
  //                 MakeCallback (&LocationTrace));

  // Connect trace source to callback functions
  // Lte
  //Config::Connect ("/NodeList/*/DeviceList/*/LteUePhy/ReportUeMeasurements",
  ...
  for (k = 0; k < users; k++)
    {
      ...
      // check if the m_id from the app match the node id from NodeContainer context
      //Ptr<Node> node = app.GetObject<>(); 
      //Ptr<MobilityModel> mob_model = node->GetObject<MobilityModel>(); 

      //std::cout << "DASH m_id: " << k << "Node_id: " << std::endl;
